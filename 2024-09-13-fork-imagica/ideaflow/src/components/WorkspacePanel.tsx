...
import { ApplyData, ChatHistory, Message } from '@/services/chatService';
...
const WorkspacePanel: React.FC = () => {
    ...
    }), [state, setActiveTab, updatePreview, addChatMessage, updateChatHistory, updateMessages, updateMergedCodeBlocks]);
        ...
        <WorkspaceContext.Provider value={contextValue}>
            <div className="flex flex-col h-full bg-white shadow-lg rounded-lg overflow-hidden">
                <div className="flex border-b bg-gray-100">
                    ...
                    <TabButton active={state.activeTab === 'codeHistory'} onClick={() => setActiveTab('codeHistory')}>代码历史</TabButton> {/* 新增Tab */}
                </div>
                <div className="flex-1 overflow-hidden">
                    {state.activeTab === 'preview' && <WorkspacePreview />}
                    {state.activeTab === 'chat' && <WorkspaceChat />}
                    {state.activeTab === 'codeHistory' && <WorkspaceCodeHistory />} {/* 新增代码历史Tab内容 */}
                </div>
            </div>
